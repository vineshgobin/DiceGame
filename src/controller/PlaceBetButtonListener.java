		...
		if (bet != null) {
			try {
				...
				Player[] players = new Player[] {
						new SimplePlayer("1", mainFrame.getPlayerPanel().getPlayerName(), betInt) };
				// pass the bet to the gameEngine

				for (Player player : players) {
					if (gameEngine.placeBet(player, betInt)) {
						// update GUI
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								...
								mainFrame.getTableAndToolbarContainerPanel().getToolBar().getRollPlayerButton()
										.setEnabled(true);
								...
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getGameStatusLabel().setText("Roll the dice to continue");
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getPlayerResultLabel().setText("Player result: N/A");
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getPlayerResultLabel().setVisible(false);
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getHouseResultLabel().setText("House result: N/A");
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getHouseResultLabel().setVisible(false);
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getGameResultLabel().setText("Winner: N/A");
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getGameStatusPanel()
										.getGameResultLabel().setVisible(false);
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getDicePanel()
										.getDice1().setVisible(false);
								mainFrame.getTableAndToolbarContainerPanel().getGameTablePanel().getDicePanel()
										.getDice2().setVisible(false);
							}
						});
					} else {
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								mainFrame.getTableAndToolbarContainerPanel().getToolBar().getBetTextField()
										.setText(null);
							}
						});
						...
						JOptionPane.showMessageDialog(mainFrame,
								"Bet must not be less than zero or greater than points.", "Invalid bet", 0);
